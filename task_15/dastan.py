...
class Dastan:
    ...
    def __CheckSquareIsValid(self, SquareReference, StartSquare):
        ...
        # we want to grab the square
        square = self._Board[self.__GetIndexOfSquare(SquareReference)]

        if square.ContainsBarrier():
            return False

        PieceInSquare = square.GetPieceInSquare()
    ...
    # method to check if barrier position is legal
    def __CheckBarrierIsValid(self, SquareReference):
        if not self.__CheckSquareInBounds(SquareReference):
            return False
    ...
    # method to place a new barrier
    # always horizontal
    # player should enter the centre square
    def PlaceBarrier(self, CenterSquareReference, P, S):
        if not self.__CheckBarrierIsValid(CenterSquareReference):
            return False

        # if the centre square reference ends in a 1 or the last column (or further), it's invalid
        # the row must be valid already due to the check above
        if CenterSquareReference % 10 == 1 or CenterSquareReference % 10 >= self._NoOfColumns:
            return False

        left_ref = CenterSquareReference - 1
        right_ref = CenterSquareReference + 1

        if not self.__CheckBarrierIsValid(left_ref) or not self.__CheckBarrierIsValid(right_ref):
            return False

        # change all squares to barriers
        self._Board[self.__GetIndexOfSquare(left_ref)] = Barrier(P, S)
        self._Board[self.__GetIndexOfSquare(CenterSquareReference)] = Barrier(P, S)
        self._Board[self.__GetIndexOfSquare(right_ref)] = Barrier(P, S)

        return True

    # this replaces CheckPlayerMove(), so we'll copy all the logic over from that and expand on it
    def CheckManhattanDistance(self, Pos, StartSquareReference, FinishSquareReference):
        if not self._CurrentPlayer.CheckPlayerMove(Pos, StartSquareReference, FinishSquareReference):
            return False

        # now we just want to find all roundabout routes
        # for example, if we give 12 and 34 we want to find 12->13->14->24->34 and 12->22->32->33->34
        # we can do the former by replacing the units digit with all the steps, and the latter by the same principle
        # coordinates are given [column][row]

        # start with vertical -> horizontal
        Route1 = True

        ColStep = -1 if StartSquareReference//10 > FinishSquareReference//10 else 1

        for col in range(StartSquareReference//10, FinishSquareReference//10, ColStep):
            print(col*10 + StartSquareReference%10)
            if self._Board[self.__GetIndexOfSquare(col*10 + StartSquareReference%10)].ContainsBarrier():
                Route1 = False

        RowStep = -1 if StartSquareReference%10 > FinishSquareReference%10 else 1

        for row in range(StartSquareReference%10, FinishSquareReference%10+RowStep, RowStep):
            if self._Board[self.__GetIndexOfSquare((FinishSquareReference//10)*10 + row)].ContainsBarrier():
                Route1 = False
            print((FinishSquareReference//10)*10 + row)

        # Route2 - horizontal -> vertical - 12->13->14->24->34
        print('route 2')
        Route2 = True

        RowStep = -1 if StartSquareReference % 10 > FinishSquareReference % 10 else 1

        for row in range(StartSquareReference % 10, FinishSquareReference % 10, RowStep):
            if self._Board[self.__GetIndexOfSquare((StartSquareReference // 10) * 10 + row)].ContainsBarrier():
                Route2 = False
            print((StartSquareReference // 10) * 10 + row)

        ColStep = -1 if StartSquareReference // 10 > FinishSquareReference // 10 else 1

        for col in range(StartSquareReference // 10, FinishSquareReference // 10+ColStep, ColStep):
            print(col * 10 + FinishSquareReference % 10)
            if self._Board[self.__GetIndexOfSquare(col * 10 + FinishSquareReference % 10)].ContainsBarrier():
                Route2 = False
    ...
    def PlayGame(self):
        ...
        # place barriers
        p1_barrier = int(input("Player 1 barrier position: "))

        while not self.PlaceBarrier(p1_barrier, self._Players[0], 'B'):
            p1_barrier = int(input("Invalid barrier position, please choose a new one: "))

        p2_barrier = int(input("Player 2 barrier position: "))

        while not self.PlaceBarrier(p2_barrier, self._Players[1], 'b'):
            p2_barrier = int(input("Invalid barrier position, please choose a new one: "))
        ...
        while not GameOver:
            ...
            MoveLegal = self.CheckManhattanDistance(Choice, StartSquareReference, FinishSquareReference)
            if MoveLegal:
...
class Square:
    ...
    # Check if the square contains a barrier or not
    def ContainsBarrier(self):
        if self._Symbol == "B" or self._Symbol == "b":
            return True
        else:
            return False
