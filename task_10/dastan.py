    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3) or 7 to replace piece with kotla or 9 to take the offer: "))

                if Choice == 7:
                    # replace piece with Kotla
                    while True:
                        SquareReference = self.__GetSquareReference("containing the piece to replace")

                        if not self.__CheckSquareIsValid(SquareReference, True):
                            continue

                        # check that square has a piece
                        square = self._Board[self.__GetIndexOfSquare(SquareReference)]

                        # check that piece is not a Kotla
                        if square.ContainsKotla():
                            continue

                        piece = square.GetPieceInSquare()

                        # make sure piece is owned by player
                        if not piece or piece.GetBelongsTo() != self._CurrentPlayer:
                            continue
                        ...
                        if self._CurrentPlayer.SameAs(self._Players[0]):
                            self._Board[self.__GetIndexOfSquare(SquareReference)] = Kotla(self._CurrentPlayer, "K")
                        else:
                            self._Board[self.__GetIndexOfSquare(SquareReference)] = Kotla(self._CurrentPlayer, "k")
