...
class Dastan:
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                Choice = int(input("Choose move option to use from queue (1 to 3) or 6 to modify options or 9 to take the offer: "))

                if Choice == 6:
                    self.__ModifyQueueOptions()
    ...
    def __ModifyQueueOptions(self):
        print("Options")
        print("a) Reverse the current player queue")
        print("b) Swap the current player queue with the opponent queue")
        print("c) Swap the first and last elements in the current player queue")
        print("d) Move one of the move options to the end of the current player queue")
        print("e) Nothing (make normal move)")

        answer = input("> ").lower()
        while answer not in list("abcde"):
            answer = input("> ").lower()
        ...
        self._CurrentPlayer.ChangeScore(-3)

        if answer == "a":
            self._CurrentPlayer.ReverseQueue()
        elif answer == "b":
            # swap the current player queue with the opponent queue
            # we don't need to use _CurrentPlayer because there are two players! huge!
            self._Players[1].ReplaceQueue(self._Players[0].ReplaceQueue(self._Players[1].GetQueue()))
        elif answer == "c":
            self._CurrentPlayer.SwapFirstAndLast()
        elif answer == "d":
            pos = int(input("Enter the position of the move option to move to the end of the queue: "))
            self._CurrentPlayer.MoveItemToFront(pos)
...
class MoveOptionQueue:
    ...
    def SwapFirstAndLast(self):
        self.__Queue[0], self.__Queue[-1] = self.__Queue[-1], self.__Queue[0]

    def MoveItemToFront(self, Position):
        self.__Queue = list(self.__Queue[Position]) + self.__Queue[:Position] + self.__Queue[Position + 1:]
...
class Player:
    ...
    def ReverseQueue(self):
        self.__Queue.ReverseQueue()

    def SwapFirstAndLast(self):
        self.__Queue.SwapFirstAndLast()

    def MoveItemToFront(self, Position):
        self.__Queue.MoveItemToFront(Position)

    # zigzag didn't really think this through
    # we need a GetQueue() method anyway
    def GetQueue(self):
        return self.__Queue
    def ReplaceQueue(self, queue):
        temp = self.__Queue
        self.__Queue = queue
        return temp
