...
class Dastan:
    def __init__(self, R, C, NoOfPieces):
        ...
        self._NoOfRows = R
        self._NoOfColumns = C
        self._MoveOptionOfferPosition = 5       # make Sahm the first offer
    ...
    def __UseMoveOptionOffer(self):
        ...
        # now to make sure we can never get sahm again
        if self._CurrentPlayer.GetSahmStatus():
            self._MoveOptionOfferPosition = random.randint(0, 4)
        else:
            self._MoveOptionOfferPosition = random.randint(0, 5)
    ...
    def __CreateMoveOptionOffer(self):
        ...
        self._MoveOptionOffer.append("sahm")            # add sahm to the offer list
    ...
    def __CreateSahmMoveOption(self):
        # new move option for sahm
        # as mentioned in task description, the piece doesn't actually move
    ...
    def __CreateMoveOption(self, Name, Direction):
        ...
        elif Name == "cuirassier":
            return self.__CreateCuirassierMoveOption(Direction)
        else:
            return self.__CreateSahmMoveOption()                # add in sahm at end
    ...
    def __CalculateSahmMove(self, StartSquareReference):
        ...
        # arrow is fired in a straight line from StartSquareReference
        # and destroys anything in its way except a Kotla and its contents

        # so first we want to find a way to iterate through all pieces
        # note that StartSquareReference has two digits, first is row and second is column
        # we want to iterate through rows
        CurrentReference = StartSquareReference
        while 1 <= CurrentReference // 10 <= 5:
            # now we have the reference, we want to see if there is a piece in the square
            # to see how squares and their pieces are indexed, instead of trying to understand the code,
            # I will jump to __CalculatePieceCapturePoints() and copy the code from there
            square = self._Board[self.__GetIndexOfSquare(CurrentReference)]
            piece = square.GetPieceInSquare()

            if piece is not None:
                if piece.GetBelongsTo() != self._CurrentPlayer and not square.ContainsKotla():
                    # if the piece belongs to the other player and the square doesn't contain a Kotla
                    # then we want to remove the piece and add its points to the current player
                    # for removing a piece we once again copy-paste from another function, this time __UpdateBoard()
                    self._Board[self.__GetIndexOfSquare(CurrentReference)].RemovePiece()

            # shift the row up or down by 1, depending on player direction
            CurrentReference += self._CurrentPlayer.GetDirection() * 10
...
class Player:
    ...
    # getter and setter for SahmUsed
    def GetSahmStatus(self):
        return self.__SahmUsed

    def SetSahmUsed(self):
        self.__SahmUsed = True
    ...
    # function to check if a choice is a Sahm or not
    # currently unsure how to implement this
    # but there must be some function that checks the current move?
    # it's very weird because the task implies it's a method in `Player`, but that makes little sense to me
    # because the logic of whether it's Sahm or not is handled by MoveOption offer, not the MoveOptionQueue Player has

    # it wants it called in PlayGame(), so the only thing it can be is literally checking if it's index 8?
    # but then why is it a function of `Player`?
    # so whack

    # ok nvm I misunderstood how it worked
    # we have to get the value of the MoveOptionOffer we choose
    def ChoiceIsSahm(self, choice):
        return self.__Queue.GetMoveOptionInPosition(choice-1).GetName() == "sahm"
