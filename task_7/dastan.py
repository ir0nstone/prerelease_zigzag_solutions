    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                prompt = "Choose move option to use from queue (1 to 3) or 9 to take the offer: "

                if self._CurrentPlayer.GetChoiceOptionsLeft() <= 0:
                    prompt = "Choose move option to use from queue (1 to 3): "

                Choice = int(input(prompt))

                if self._CurrentPlayer.GetChoiceOptionsLeft() > 0 and Choice == 9:
                    self.__UseMoveOptionOffer()
                    self.__DisplayState()
                    self._CurrentPlayer.DecrementChoiceOptionsLeft()
...
class Player:
    ...
    def DecrementChoiceOptionsLeft(self):
        self.__ChoiceOptionsLeft -= 1
        print("Choice options left:", self.__ChoiceOptionsLeft)
