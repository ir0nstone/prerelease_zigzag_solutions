...
class Dastan:
    def __init__(self, R, C, NoOfPieces):
        ...
        self.__NoOfPieces = NoOfPieces + 1      # to include mirza
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            if MoveLegal:
                # here?
                self.__CheckReincarnation(FinishSquareReference)
    ...
    def __CountNormalPieces(self):
        ...
        # easiest way is to probs just iterate through all
        ...
        for r in range(1, 7):
            for c in range(1, 7):
                ...
                piece = self._Board[self.__GetIndexOfSquare(index)].GetPieceInSquare()

                if piece == None:
                    continue

                if piece.GetBelongsTo().SameAs(self._CurrentPlayer):
                    count += 1

        return count

    def __CheckReincarnation(self, FinishSquareReference):
        # first calculate back row
        # to me, most logical way is to use the player's direction
        # so for p1, direction is + and back row is 6
        # for p2, back row is 1

        backrow = int(3.5 + (2.5 * self._CurrentPlayer.GetDirection()))
        frontrow = 7 - backrow

        if FinishSquareReference // 10 != backrow:
            return

        if self.__CountNormalPieces() >= self.__NoOfPieces:
            return

        # if we get here, we can reincarnate
        spawn_col = int(input("Enter the column (1-6) you wish to reincarnate to: "))
        spawn_square = 10 * frontrow + spawn_col

        while self._Board[self.__GetIndexOfSquare(spawn_square)].GetPieceInSquare() != None:
            spawn_col = int(input("Enter the column (1-6) you wish to reincarnate to: "))
            spawn_square = 10 * frontrow + spawn_col

        # copying code from __CreatePieces
        symbol = "!"

        if self._CurrentPlayer.SameAs(self._Players[1]):
            symbol = '"'

        NewPiece = Piece("piece", self._CurrentPlayer, 1, symbol)
        self._Board[self.__GetIndexOfSquare(spawn_square)].SetPiece(NewPiece)
