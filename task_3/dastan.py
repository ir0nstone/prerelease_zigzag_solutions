...
class Dastan:
    ...
    # edited here
    # we just copy-paste __CreateChowkidarMoveOption() and change the moves
    # moves are more complex here as it's not symmetrical
    # so we have to work out which parameter is x-axis, which is y
    # instead of trying to read the code, I'm going to look at the Chowdikar moves
    # because only the x-axis has a direction of 2 (look at the PDF of the prelim material to see)
    # so therefore the parameters must go y-axis then x-axis
    # and we can use jazair similarly to tell us that positive is forwards
    ...
    def __CreateMoveOptions(self):
        ...
        self._Players[0].AddToMoveOptionQueue(self.__CreateMoveOption("sarukh", 1))  # wanted it after the ryott for both players
        ...
        self._Players[1].AddToMoveOptionQueue(self.__CreateMoveOption("sarukh", -1))  # wanted it after the ryott for both players
