...
class Dastan:
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            # here is a logical place to handle Wafr, because it changes the options
            # we only want to handle it if the player has not already been awarded it
            is_wafr = self.__AwardWafr()

            if is_wafr and not self._CurrentPlayer.GetWafrAwarded():
                print("You have been awarded a Wafr, you can select any move options from your queue for free this turn")

                self._CurrentPlayer.SetWafrAwarded(True)    # make sure set

                # and now we can handle it
                while Choice < 1 or Choice > 5:
                    Choice = int(input("Choose move option to use from queue (1 to 5)"))
            else:
                # if we don't have Wafr, then we can just handle the normal move options
    ...
    # create a basic 25% chance generator
    def __AwardWafr(self):
...
class Player:
    def __init__(self, N, D):
        ...
        self.__WafrAwarded = False          # has the player been awarded a wafr yet
    ...
    # accessor and mutator for __WafrAwarded
    def GetWafrAwarded(self):
        return self.__WafrAwarded

    def SetWafrAwarded(self, awarded):
        self.__WafrAwarded = awarded
