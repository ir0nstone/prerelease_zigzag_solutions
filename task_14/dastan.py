...
class Dastan:
    def __init__(self, R, C, NoOfPieces):
        ...
        self._CurrentWeatherEvent = None
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            if self._CurrentWeatherEvent is not None:
                self._CurrentWeatherEvent.DecrementCountdown()

                if self._CurrentWeatherEvent.CountdownComplete():
                    col = self._CurrentWeatherEvent.GetWeatherLocation() % 10

                    # destroy everything on column
                    for r in range(1, self._NoOfRows+1):
                        SquareReference = 10*r + col

                        self._Board[self.__GetIndexOfSquare(SquareReference)].RemovePiece()
                        # that doesn't remove Kotla, so we do that here
                        self._Board[self.__GetIndexOfSquare(SquareReference)] = Square()

                    # reset event
                    self._CurrentWeatherEvent = None

            self.__WeatherEventOccurs()
    ...
    def __WeatherEventOccurs(self):
        if random.randint(1, 2) == 1 or self._CurrentWeatherEvent is not None:
            # not occurring
            return

        # has to be unoccupied space
        # TODO FIX
        row, col = random.randint(1, self._NoOfRows), random.randint(1, self._NoOfColumns)
        SquareReference = 10*row + col

        while self._Board[self.__GetIndexOfSquare(SquareReference)].GetPieceInSquare() is not None:
            row, col = random.randint(1, self._NoOfRows), random.randint(1, self._NoOfColumns)
            SquareReference = 10*row + col

        # 4 because we are counting each turn as a round because then we can decrement once per loop
        # changeme
        self._CurrentWeatherEvent = WeatherEvent(2, SquareReference)

        print("Weather Event occurring at position", SquareReference, "in 2 rounds!")
...
class WeatherEvent:
    def __init__(self, Countdown, WeatherLocation):
        self._Countdown = Countdown

        # why is this not mentioned in the instantiation part?
    ...
    def SetWeatherLocation(self, NewLocation):
        self._WeatherLocation = NewLocation
