    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            if MoveLegal:
                ...
                PriorScore = self._CurrentPlayer.GetScore()
                ...
                # now ask if we want an Undo
                undo = input("Undo (y/n)?")

                if undo.lower() == 'y':
                    self.__UpdateBoard(FinishSquareReference, StartSquareReference)
                    self._CurrentPlayer.ChangeScore(PriorScore - 5)
...
class MoveOptionQueue:
    ...
    # adding in feature to roll queue back
    # we keep the list up until the position itself
    # then we add the move back into its position (the move is located at the end of the list once played)
    # then add on the remainder of the original list, not including the very last one
    def ResetQueueBack(self, Position):
        self.__Queue = self.__Queue[:Position] + [self.__Queue[-1]] + self.__Queue[Position:-1]
