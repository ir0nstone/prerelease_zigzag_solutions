...
class Dastan:
    ...
    def __UseMoveOptionOffer(self):
        # edited to read a valid integer
        ...
        while not 1 <= ReplaceChoice <= 5:
            ReplaceChoice = self.__GetValidInt("Invalid choice, please try again: ",
                                               "Invalid choice, please try again: ")
    ...
    def PlayGame(self):
        ...
        while not GameOver:
            ...
            while Choice < 1 or Choice > 3:
                # read integer here
                Choice = self.__GetValidInt("Choose move option to use from queue (1 to 3) or 9 to take the offer: ",
                                            "Invalid choice, please try again: ")
                if Choice == 9:
    ...
    # Task description is really vague, so I'm interpreting it the way that makes most sense to me
    # in this context, I feel like this makes the most sense
    def __GetValidInt(self, prompt, error):
        value = input(prompt)
        while True:
            try:
                value = int(value)
